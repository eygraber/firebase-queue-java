...
class QueueTask implements Runnable {
  static final String PRE_RUN_ID = "<id not set yet>";
  private String id;
  ...
  QueueTask(@NotNull Firebase taskRef, @NotNull TaskSpec taskSpec, @NotNull TaskReset taskReset, @NotNull Queue.Options options) {
  ...
  @Override
  public void run() {
    id = Thread.currentThread().getName() + ":" + UuidUtils.randomUUID();
